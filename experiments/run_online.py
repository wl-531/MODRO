...
def run_experiment(n_batches: int = 10, verbose: bool = True):
    ...
    print(f"聚合标准差: {validation['total_std']:.0f}")
    print(f"鲁棒负载: {validation['robust_load']:.0f}")
    print(f"期望负载率 ρ: {validation['rho_expected']:.3f}")
    print(f"鲁棒负载率 ρ_robust: {validation['rho_robust']:.3f}")
    print(f"有效负载率 ρ_eff (θ={THETA}): {validation['rho_effective']:.3f}")
    print(f"系统可行性: {'可行' if validation['feasible'] else '不可行'}")
    print()
